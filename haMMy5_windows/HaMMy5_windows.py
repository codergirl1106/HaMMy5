...
defaultFont.configure(family="Adobe Devanagari", size=12)
...
def refresh(*args):
    ...
    if num_of_states.get() > 10 or num_of_states.get() < 2:
        ...
        if file != "":
            aa.clear()
            if c1.get():
                subprocess.call([f"exec.exe", f"{num_of_states.get()}", f"{file}", f"{' '.join(map(lambda x: x.get(), optionsss))}"])
            else:
                subprocess.call([f"exec.exe", f"{num_of_states.get()}", f"{file}"])
...
def upload(event=None):
    ...
    if num_of_states.get() > 10 or num_of_states.get() < 2:
        ...
        try:
            for ff in filenames:
                if c1.get():
                    subprocess.call([f"exec.exe", f"{num_of_states.get()}", f"{ff}", f"{' '.join(map(lambda x: x.get(), optionsss))}"])
                else:
                    subprocess.call([f"exec.exe", f"{num_of_states.get()}", f"{ff}"])
...
def save_g(event=None):
    ...
    os.system(f'del "{os.path.basename(file)[:-4]}_haMMyfit.png"')
    os.system(f'del "{os.path.basename(file)[:-4]}_fluorophoreintensity.png"')
...
button3.grid(row=2,column=7,rowspan=2, sticky=tk.S, padx=(46, 0))

button2 = tk.Button(window, text='↻', command=alll, width=3,height=3)
button2.grid(row=2,column=7,rowspan=2, sticky=tk.S, padx=(190, 0))
